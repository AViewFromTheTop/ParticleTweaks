...
It allows to separate the actual name of the variable and the namespace, `particleTweaks`.

Fields marked as `@Unique` must be prefixed with `particleTweaks$` or `PARTICLETWEAKS$` if the field is static and final.
Methods marked with `@Unique` do not need prefixes.


## Licensing & DCO

Particle Tweaks is licensed under [GPL 3.0][LICENSE], and it has a [Developer Certificate of Origin (DCO)][DCO], which you are required to agree with to contribute.
...
[LICENSE]: ./LICENSE "Particle Tweaks license file"
